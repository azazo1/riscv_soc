...
| `0x0000_f000` - `0x0000_ffff` | boot RAM | bootloader `.bss` 和 stack, 也支持从这里取指 |
...
SDRAM 已经作为 data bus 和 instruction fetch 的大窗口接入, 不替换低地址 boot RAM.
...
CPU 看到的是统一 memory bus, 不直接关心底层是 boot RAM, SDRAM, UART, 还是 MMIO.

后续路线:

1. 用 Quartus 检查新的 SDRAM 取指布局资源占用.
2. 用 Quartus 检查 `dram_clk` 相关 timing.
3. 如果 50 MHz 稳定, 再考虑 PLL 到 100 MHz.
4. 后续可以给 SDRAM 取指增加更正式的 icache 或 burst 读取.
