...
本文记录让当前 RV32IM CPU 运行 Rust `no_std` 裸机程序前需要补齐的硬件和软件条件. 当前目标不是运行操作系统, 而是先运行一个很小的 Rust 函数, 能访问 MMIO, 能用 UART 或 LED 输出可观察结果.

## 运行目标

第一阶段目标:

- 使用 `riscv32i-unknown-none-elf` 或等价 RV32IM 裸机目标.
- 程序从 ROM 启动.
- 栈, `.data`, `.bss` 放在 RAM.
...
Rust 编译器即使在 `no_std` 下, 也会生成比较完整的 RV32I 基础指令. 当前 CPU 还支持 RV32M 乘除法扩展:
...
| 地址 | 名称 | 方向 | 有效位 | 作用 |
| --- | --- | --- | --- | --- |
| `0x0100_0100` | `UART_TXDATA` | W | `[7:0]` | 写入一个待发送字节 |
...
仿真阶段可以先检查 `tx_valid` 和 `tx_data`. 上板阶段当前把真实 UART TX 复用到 `gpio1[0]`, 外接 USB-TTL 模块时把 USB-TTL 的 RX 接到 GPIO_1[0], 并且 GND 共地.

## 不需要立即实现的能力

这些能力对第一版 Rust 裸机程序不是硬性要求:

- 多周期乘除法器. 当前先使用组合乘除法, 后续如果时序压力大再优化.
- 中断. 第一版可以轮询 MMIO.
